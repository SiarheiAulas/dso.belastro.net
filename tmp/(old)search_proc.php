Старый скрипт поиска через LIKE

<?php
$title='Результаты поиска';//для заголовка
function page(){
	include (ROOT.'/inc/db_connect.php');//для подключения к БД (используется mysqli)
	...
	$search="%$search%";//строка запроса преобразуется для последующей вставки в шаблон запроса в оператор LIKE

	//считает количество статей по запросу
	if($user_id) {//если выбран пользователь в форме поиска
		$count_articles=$mysqli->prepare("SELECT COUNT(*) FROM `articles` WHERE user_id=? AND (subject LIKE ? OR content LIKE ?)");
		//шаблон запроса
		$count_articles->bind_param('sss',$user_id, $search, $search);//привязка параметров 1
	} else { // если не выбран пользовател в форме поиска
		$count_articles=$mysqli->prepare("SELECT COUNT(*) FROM `articles` WHERE subject LIKE ? OR content LIKE ?");
		//шаблон запроса без указания пользователя
		$count_articles->bind_param('ss', $search, $search);//привязка параметров 2 
	}
	...
	if(strlen($search)<4||strlen($search)>128){ //если неправильная длинна запроса
		...
		if ($user_id){//если пользователь в форме запроса указан
			$request=$mysqli->prepare("SELECT * FROM `users` INNER JOIN `articles` ON `articles`.`user_id`=`users`.`id` 
				WHERE user_id=? AND (subject LIKE ? OR content LIKE ?) ORDER BY `publication_date` DESC LIMIT ?, ?");
			$request->bind_param('sssii', $user_id, $search, $search,$start,$limit_per_page); //привязка параметров 1 
		} else { //пользователь не указан в форме запроса
			$request=$mysqli->prepare("SELECT * FROM `users` INNER JOIN `articles` ON `articles`.`user_id`=`users`.`id` 
				WHERE subject LIKE ? OR content LIKE ? ORDER BY `publication_date` DESC LIMIT ?, ?");
			$request->bind_param('ssii', $search, $search,$start,$limit_per_page);//привязка параметров 2
		}
